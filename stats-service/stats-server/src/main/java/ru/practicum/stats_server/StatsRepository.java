...
public interface StatsRepository extends JpaRepository<Stats, Long> {
    ...
    @Query("SELECT new ru.practicum.stats_dto.model.ViewStats(s.app, s.uri, COUNT(s.ip)) " +
            "FROM Stats AS s " +
    ...
    @Query("SELECT new ru.practicum.stats_dto.model.ViewStats(s.app, s.uri, COUNT(DISTINCT s.ip)) " +
            "FROM Stats AS s " +
            "WHERE s.timestamp BETWEEN ?1 AND ?2 " +
            "GROUP BY s.app, s.uri " +
